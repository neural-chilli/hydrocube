...
async fn test_load_jsonlines_file_into_table() {
    ...
    load_file_into_table(&db, "greeks", f.path().to_str().unwrap(), DataFormat::JsonLines)
        .await
        .unwrap();

    let rows = db.query_json("SELECT COUNT(*) AS cnt FROM greeks", vec![]).await.unwrap();
...
async fn test_load_csv_file_into_table() {
    let db = DbManager::open_in_memory().unwrap();
    db.execute(
        "CREATE TABLE prices (ticker VARCHAR, price DOUBLE)",
        vec![],
    ).await.unwrap();
    ...
    let rows = db.query_json("SELECT COUNT(*) AS cnt FROM prices", vec![]).await.unwrap();
    assert_eq!(rows[0]["cnt"].as_u64().unwrap(), 2);
}
...
async fn test_load_nonexistent_file_returns_error() {
    let db = DbManager::open_in_memory().unwrap();
    db.execute("CREATE TABLE t (x VARCHAR)", vec![]).await.unwrap();
    let result = load_file_into_table(&db, "t", "/nonexistent/path/file.json", DataFormat::Json).await;
