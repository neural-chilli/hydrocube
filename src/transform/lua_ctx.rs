...
pub fn run_lua_with_identity_ctx(
    ...
    let mark_seen_out = lua
        .create_table()
        .map_err(|e| HcError::Transform(e.to_string()))?;
    lua.globals()
        .set("__mark_seen_out", mark_seen_out)
        .map_err(|e| HcError::Transform(e.to_string()))?;

    // Build __seen_set as a Lua table snapshot: {key = true, ...}
    let seen_lua: Table = lua
        .create_table()
        .map_err(|e| HcError::Transform(e.to_string()))?;
    ...
    let seen_fn = lua
        .create_function(|lua_ctx, key: String| {
    ...
    let mark_out: Table = lua
        .globals()
        .get("__mark_seen_out")
        .map_err(|e| HcError::Transform(e.to_string()))?;
    let marks: Vec<String> = mark_out
